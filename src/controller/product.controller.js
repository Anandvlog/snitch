...
import { uploadeFile } from "../services/storage.service.js";
...
export async function createProduct(req, res) {
  ...
  if (user.role !== "seller") {
    return res.status(403).json({
      message: "Only sellers can create produts",
    });
  }
  ...
  if (!files || files.length === 0) {
    errors.push({
      field: "images",
      message: "At least one image is requred",
    });
  }
  ...
  const urls = await Promise.all(
    req.files.map(async (file, index) => {
      ...
      const response = await uploadeFile(
